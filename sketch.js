...
function draw() {
  background('#ffffff');
  imageMode(CENTER);
  image(qr_Reset, width *0.05, height *0.55, qrSize/2, qrSize/2);
  ...
  if (!qr2Scanned) image(qr2, width * 0.40, height * 0.3, qrSize, qrSize);


  if (!qr3Scanned)  image(qr3, width * 0.60, height * 0.3, qrSize, qrSize);
  
  if (!qr4Scanned) image(qr4, width * 0.80, height * 0.3, qrSize, qrSize);

  //second row yo
  

  if (!qr5Scanned) image(qr5, width * 0.20, height * 0.80, qrSize, qrSize);

  if (!qr6Scanned) image(qr6, width * 0.40, height * 0.80, qrSize, qrSize); 
  
  if (!qr7Scanned) image(qr7, width * 0.60, height * 0.80, qrSize, qrSize); 
  
  if (!qr8Scanned) image(qr8, width * 0.80, height * 0.80, qrSize, qrSize); 
  
  if (qr1Scanned && qr2Scanned && qr3Scanned && qr4Scanned && qr5Scanned && qr6Scanned && qr7Scanned && qr8Scanned ) {
    image(qr_Correct, width * 0.5, height * 0.5, 300, 300)
    ...
    fill(0); 
    textFont('Alumni Sans SC')
    text("Well done!\nScan the QR Code with your phone!", width / 2, height / 4);
    pop();
...
function codeScanned(text) {
  switch (text) {
    case "https://www.ntu.edu.sg/":
      ...
      fireworks.at(-1).trigger(width * 0.20, height * 0.3);
      break;
      
    case "https://www.nus.edu.sg/":
      qr2Scanned = true;
      fireworks.push(new Firework());
      fireworks.at(-1).trigger(width * 0.40, height * 0.3);
      break;
      
    case "https://www.suss.edu.sg/":          
      qr3Scanned = true;
      fireworks.push(new Firework());
      fireworks.at(-1).trigger(width * 0.60, height * 0.3);
  break;

    case "https://www.sutd.edu.sg/":          
      qr4Scanned = true;
      fireworks.push(new Firework());
      fireworks.at(-1).trigger(width * 0.80, height * 0.3);
  break;

    case "https://www.smu.edu.sg/":          
      qr5Scanned = true;
      fireworks.push(new Firework());
      fireworks.at(-1).trigger(width * 0.20, height * 0.80);
  break;
